...
1. QUIC() function must be running in matlab. Please download the [MEX package archive v1.2](http://www.cs.utexas.edu/~sustik/QUIC/QUIC_MEX_1.2.tar) from http://www.cs.utexas.edu/~sustik/QUIC/, unzip it, and compile it following the instructions in README file inside QUIC package. By default, QUIC package is expected to be inside the _twn directory_ as a folder named _QUIC_, but you may install it in any other directory and configure it in the _settings.sh_ file in the _twn directory_.
